...
- **Never build the full service×city matrix by default.** The consensus-safe pattern is one page per service plus a small set of city pages; `/services/[service]/[city]` combo URLs are reserved for the 1-3 MONEY services in cities where the local-material gate passes (real jobs, photos, reviews from there). [CONVENTION, strong consensus]
