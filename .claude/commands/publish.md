...
**One page goes live immediately; a launch ships together.** There is no queue and no scheduler in this lane - what you publish is live on the next deploy. See `references/publishing-cadence.md` for how fast that is allowed to be.

**Pre-flight - refuse politely if these fail:**
1. Drafts being published are Status = Linked in `website-index.md` (never publish an orphan - every page needs at least two inbound links from existing pages)
...
3. Search Console: Already set up → just confirm the sitemap reflects the new pages and request indexing on the biggest new URLs via URL Inspection.
