...
**Ask for more than the site needs today.** A page needs three photographs; a member who sends thirty never gets asked again, and `/service-page`, `/blog-post` and `/gbp-posts` all draw from the same library for months. Running out of photos in week three is what sends a site back to stock.
