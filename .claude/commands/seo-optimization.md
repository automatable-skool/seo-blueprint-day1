...
- **Request re-indexing.** If metadata, schema, or structure changed, the page needs a recrawl: GSC → URL Inspection → paste the URL → Request indexing (paste the URL into Search Console by hand). Say plainly that recrawls take days, not hours - the log's re-measure date is when the numbers mean something.
