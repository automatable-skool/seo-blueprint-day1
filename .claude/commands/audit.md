...
**If there is no Search Console property at all**, that is a critical finding on its own and goes near the top of the report. Set it up by hand: verify the property, submit the sitemap, request indexing.
...
- **No Search Console property, or a property nobody has looked at (Layer 1)** → set up Search Console by hand
...
- Structure and merge/redirect plans → `/build-website`
