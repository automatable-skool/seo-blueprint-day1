...
**Guard 1 - the sweep only ever promotes. Standalone → Spoke, never the reverse.** The decompose test is a judgment call, not a deterministic check, so an unguarded sweep lets a page flip standalone → spoke on one run and back on the next. That is not a cosmetic wobble: the hub will already carry a written passage for it, and demoting the page leaves that hub with a paragraph about something that is no longer its spoke, which nothing cleans up. So **this step never demotes anything.** A page leaves a hub only by explicit human decision, never by sweep.
...
- **A promotion is a real change downstream.** the linking pass reads these labels, so a page moving into a hub means that hub's section grows a passage for it on the next linking run - the mechanism that keeps the graph current instead of frozen at first pull. Say so when reporting the move.

**Report the moves in chat, never silently.** One line each: `"citation audit" was standalone, now a spoke of hub 3 (local SEO audit) - grow the hub's section for it when you next touch that page`. If nothing moved, say `standalone sweep: 12 checked against 3 open hubs, none cluster yet` in one line. Never a table, never a per-page essay.
...
**E5. Report:** pipeline size, months of content at my pace (ask if unknown), and the 10 best pages I'm not building yet with one line each on why they'd win. The pipeline is only worth what gets written.
